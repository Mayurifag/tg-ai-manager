...
class TelegramMessageParser:
    def __init__(self, client: Any, media_component: Any):
        self.client = client
    ...
    async def parse(
        ...
        if has_media:
            if isinstance(media, MessageMediaPoll):
                is_poll = True
                poll_question = getattr(media.poll.question, "text", "Poll")
        ...
        if sender:
            ...
            avatar_url = await self.media.get_chat_image_url(sender, sender_id)
        ...
        if reply_header:
            ...
            if reply_to_msg_id and reply_to_msg_id in replies_map:
                ...
                r_s = getattr(r_msg, "sender", None)
                reply_to_sender = utils.get_display_name(r_s) if r_s else "User"
